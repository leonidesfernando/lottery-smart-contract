...
contract Lottery {
    ...
    function bet(address payable[] memory gamblers) public payable{
        for(uint i = 0; i < gamblers.length; i++){
            bet(gamblers[i]);
        }
    }

    function bet(address payable player) public payable{
        require(status == BetsState.Open, 'The raffle must be open to bet');
        require(address(this).balance < TICKET_PRICE, 'The minimum ticket price (lot) is 0.5 Ether.');
